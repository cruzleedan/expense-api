...
├── db/drizzle.ts       # Drizzle instance (shares same pg pool)
├── db/schema.ts        # Drizzle table definitions for all tables (TypeScript types)
├── db/schema.sql       # Plain SQL DDL — source of truth for migrations (v5.0)
...
| Drizzle instance | `src/db/drizzle.ts` |
| Drizzle table types | `src/db/schema.ts` |
...
| Full DB schema (DDL) | `src/db/schema.sql` |
...
|---|---|---|
| Code review | `context/agents/expense-api-reviewer.md` | Checks Hono patterns, Drizzle/SQL split, AppError usage, TypeScript conventions |
